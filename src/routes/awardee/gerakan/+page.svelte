<script>
	...
	function resetProposal() { proposal = { title: '', category: 'LINGKUNGAN', objective: '', description: '', region: '', startsAt: '', endsAt: '', targetParticipants: 10 }; }
	async function propose() { working = true; error = ''; try { await proposeMovement({ ...proposal, targetParticipants: Number(proposal.targetParticipants), startsAt: new Date(proposal.startsAt).toISOString(), endsAt: new Date(proposal.endsAt).toISOString() }); proposalOpen = false; resetProposal(); success = 'Usulan Gerakan berhasil dikirim kepada Verifikator.'; await load(); } catch (exception) { error = exception.message; } finally { working = false; } }
	...
	async function report(event) { working = true; error = ''; try { await submitMovementReport(reportFor.id, new FormData(event.currentTarget)); reportFor = null; success = 'Laporan aksi berhasil dikirim kepada Verifikator.'; await load(); } catch (exception) { error = exception.message; } finally { working = false; } }
	onMount(load);
</script>
...
<section class="rounded-card border border-brand-200 bg-brand-50 px-5 py-5 sm:px-6" aria-labelledby="movement-action-title">
	<div class="flex flex-wrap items-center justify-between gap-4">
		...
		<div class="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row"><Button size="lg" iconPath={ICONS.plus} onclick={() => proposalOpen = true}>Ajukan Gerakan Baru</Button>{#if needingReport.length > 0}<Button size="lg" variant="secondary" onclick={() => activeTab = 'joined'}>Lihat Gerakan saya</Button>{/if}</div>
	</div>
</section>
	...
	<div class="mt-6 space-y-4">
		{#each shown as movement}
			<Card>
				...
				<div class="mt-4 flex flex-wrap items-center gap-2">{#if movement.status === 'BERJALAN' && !movement.myParticipant}<Button size="sm" onclick={() => join(movement.id)} disabled={working}>Gabung Gerakan</Button>{/if}{#if movement.status === 'BERJALAN' && movement.myParticipant}<Button size="sm" onclick={() => reportFor = movement}>Kirim laporan aksi</Button><StatusBadge label={movement.myParticipant.role === 'LEADER' ? 'Pemimpin' : 'Peserta'} color="navy" />{/if}</div>
				{#if movement.reports?.length}<div class="mt-4 border-t border-ink-100 pt-4"><h3 class="text-sm font-semibold text-heading">Riwayat laporan saya</h3><div class="mt-3 grid gap-2">{#each movement.reports as report}<div class="flex items-center justify-between gap-3 rounded-lg bg-ink-50 p-3"><div><p class="text-sm font-medium text-heading">Aksi {date(report.activityDate)}</p><p class="text-xs text-ink-500">{report.location}{#if report.awardedPoints} · {report.awardedPoints} poin{/if}</p></div><StatusBadge label={statusMeta[report.status]?.label || report.status} color={statusMeta[report.status]?.color || 'slate'} /></div>{/each}</div></div>{/if}
			</Card>
		{/each}
	</div>
{/if}

<Modal open={proposalOpen} title="Ajukan Gerakan" size="lg" onclose={() => proposalOpen = false}>
	<form class="grid gap-4 md:grid-cols-2" onsubmit={(event) => { event.preventDefault(); propose(); }}>
		...
		<div class="flex flex-wrap gap-2 md:col-span-2"><Button type="submit" loading={working}>Kirim ke Verifikator</Button><Button variant="secondary" onclick={() => proposalOpen = false}>Batal</Button></div>
	</form>
</Modal>

<Modal open={Boolean(reportFor)} title="Kirim laporan aksi" size="md" onclose={() => reportFor = null}>
	{#if reportFor}<form class="grid gap-4" onsubmit={(event) => { event.preventDefault(); report(event); }}><p class="text-sm text-ink-600">Gerakan: <strong>{reportFor.title}</strong></p><label class="grid gap-1.5 text-sm font-medium text-ink-700">Tanggal aksi<input class="rounded-xl border border-ink-200 px-3 py-2.5" name="activityDate" type="date" required></label><label class="grid gap-1.5 text-sm font-medium text-ink-700">Lokasi<input class="rounded-xl border border-ink-200 px-3 py-2.5" name="location" required></label><label class="grid gap-1.5 text-sm font-medium text-ink-700">Jumlah peserta<input class="rounded-xl border border-ink-200 px-3 py-2.5" name="participantCount" type="number" min="1" required></label><label class="grid gap-1.5 text-sm font-medium text-ink-700">Hasil aksi<textarea class="rounded-xl border border-ink-200 px-3 py-2.5" name="outcomeNote" rows="5" minlength="40" required></textarea></label><label class="grid gap-1.5 text-sm font-medium text-ink-700">Bukti foto atau PDF<input class="rounded-xl border border-dashed border-ink-300 p-3 text-sm" name="evidenceFiles" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required multiple><span class="text-xs font-normal text-ink-500">Maksimum 5 file, masing masing 5 MB.</span></label><div class="flex flex-wrap gap-2"><Button type="submit" loading={working}>Kirim laporan</Button><Button variant="secondary" onclick={() => reportFor = null}>Batal</Button></div></form>{/if}
</Modal>
