...
export async function listAdminAwardees(filters = {}) {
	try {
		...
		return await client().send(`/api/pfriends/admin/awardees?${params}`);
...
export async function adminAwardeeHistory(id) {
	try { return await client().send(`/api/pfriends/admin/awardees/${encodeURIComponent(id)}/history`); }
...
export async function changeAdminAwardeeStatus(id, kind, status, reason) {
	...
	try { return await client().send(`/api/pfriends/admin/awardees/${encodeURIComponent(id)}/${segment}`, { method: 'POST', body: { status, reason } }); }
...
export async function beginAwardeeImpersonation(id) {
	try { return await client().send(`/api/pfriends/admin/awardees/${encodeURIComponent(id)}/impersonate`, { method: 'POST' }); }
...
export async function endAwardeeImpersonation(id) {
	try { return await client().send(`/api/pfriends/admin/impersonations/${encodeURIComponent(id)}/end`, { method: 'POST' }); }
