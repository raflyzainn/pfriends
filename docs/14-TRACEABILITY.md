...
| **Forum PFriends** | Forum persisten lintas Awardee, Verifikator, dan Admin dengan picker seluruh emoji Unicode serta reply inline | `pocketbase/pb_hooks/forum.pb.js` · `src/lib/stores/forum.svelte.js` · `routes/{awardee,verifikator,admin}/forum/+page.svelte` | `npm run verify:forum` menguji pembatasan komunitas, kanal pengumuman, identitas server, idempotensi, reply dan konteks, allowlist reaksi, heartbeat presence, dan SSE |
