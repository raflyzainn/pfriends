...
| **Forum PFriends** | Forum persisten lintas Awardee, Verifikator, dan Admin tanpa mengubah komposisi visual | `pocketbase/pb_hooks/forum.pb.js` · `src/lib/stores/forum.svelte.js` · `routes/{awardee,verifikator,admin}/forum/+page.svelte` | `npm run verify:forum` menguji pembatasan komunitas, kanal pengumuman, identitas server, idempotensi, REST, reaksi, heartbeat presence, dan SSE |
