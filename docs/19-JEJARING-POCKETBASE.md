...
Pencarian, filter komunitas/chapter/kota/keahlian/mentor, statistik, serta pagination dijalankan server. Poin, tier, streak harian, dan badge ringkas direkonsiliasi dari gamifikasi PocketBase. `streakDays` dihitung dari hari masuknya poin sah dalam WIB dan dinolkan saat proyeksi lama dibaca setelah streak putus. Profil Jejaring milik sendiri dapat diperbarui melalui `PATCH /api/pfriends/profile/me`.
...
Verifikasi utama dijalankan dengan `npm run verify:directory` dan `npm run verify:streak` setelah PocketBase aktif dan seed selesai.
