...
routerAdd('POST', '/api/pfriends/stories/drafts', (e) => {
	...
	e.app.runInTransaction((tx) => {
		const awardee = tx.findFirstRecordByData('awardees', 'user', e.auth.id); const consent=Boolean(require(`${__hooks}/awardee-profile-utils.js`).effective(tx,awardee,'PUBLIKASI_CERITA')); const row = new Record(tx.findCollectionByNameOrId('stories')); const key = `${e.auth.id}-${Date.now()}`;
		row.set('legacyId', `DRAFT-${key}`); row.set('slug', `draft-${key}`); row.set('author', awardee.id); row.set('owner', e.auth.id); row.set('authorLegacyId', awardee.getString('legacyId')); row.set('authorName', awardee.getString('fullName')); row.set('status', 'DRAFT'); row.set('community', awardee.getString('community')); row.set('chapterId', awardee.getString('chapterId')); row.set('sensitivityScan', 'MENUNGGU'); row.set('consentActive', consent); row.set('consentLegacyId', consent ? `PROFILE-${awardee.getString('legacyId')}` : ''); row.set('revisionCount', 0); row.set('awardedPoints', 0); utils.applyDraft(row, data, evidence, covers); tx.save(row); utils.statusEvent(tx, row, e.auth, 'DRAFT_SAVED', '', 'DRAFT', ''); result = row;
	});
...
routerAdd('POST', '/api/pfriends/stories/{id}/submit', (e) => {
	...
	e.app.runInTransaction((tx) => {
		...
		const awardee = tx.findFirstRecordByData('awardees', 'user', e.auth.id); if (!require(`${__hooks}/awardee-profile-utils.js`).effective(tx,awardee,'PUBLIKASI_CERITA')) throw new BadRequestError('Consent publikasi Cerita belum aktif.'); utils.validateSubmission(row);
		...
		let points = row.getInt('awardedPoints'); if (before === 'DRAFT') { try { tx.findFirstRecordByFilter('verified_point_activities', 'story = {:story} && (actionCode = "STORY_SUBMIT" || activityType = "STORY_SUBMIT")', { story: row.id }); } catch (_) { const actionUtils=require(`${__hooks}/point-action-utils.js`),action=actionUtils.byCode(tx,'STORY_SUBMIT',true),date = now.slice(0,10); const count = tx.findRecordsByFilter('verified_point_activities', 'awardeeId = {:awardee} && (actionCode = "STORY_SUBMIT" || activityType = "STORY_SUBMIT") && occurredAt >= {:start} && occurredAt <= {:end}', '', 0, 0, { awardee: awardee.getString('legacyId'), start: `${date} 00:00:00.000Z`, end: `${date} 23:59:59.999Z` }).length; points = count >= action.getInt('dailyCap') ? 0 : action.getInt('points'); const ledger = new Record(tx.findCollectionByNameOrId('verified_point_activities')); ledger.set('user', e.auth.id); ledger.set('source', 'STORY'); ledger.set('story', row.id); ledger.set('awardeeId', awardee.getString('legacyId')); actionUtils.attach(ledger,action); ledger.set('points', points); ledger.set('capReason', points ? '' : 'DAILY_CAP'); ledger.set('occurredAt', now); ledger.set('awardedAt', now); ledger.set('status', 'AWARDED'); tx.save(ledger); require(`${__hooks}/gamification-utils.js`).ensureProfile(tx, awardee); } }
...
routerAdd('POST', '/api/pfriends/verifier/stories/{id}/decision', (e) => {
	...
	e.app.runInTransaction((tx) => {
		...
		if (body.decision === 'APPROVE' && !require(`${__hooks}/awardee-profile-utils.js`).effective(tx,awardee,'PUBLIKASI_CERITA')) throw new BadRequestError('Consent penulis sudah tidak aktif.');
...
routerAdd('POST', '/api/pfriends/stories/consent/revoke', (e) => {
	const utils = require(`${__hooks}/story-utils.js`); utils.requireRole(e, 'AWARDEE'); const profile=require(`${__hooks}/awardee-profile-utils.js`);let result;e.app.runInTransaction((tx)=>{const awardee=tx.findFirstRecordByData('awardees','user',e.auth.id);profile.appendConsent(tx,awardee,e.auth,'PUBLIKASI_CERITA','REVOKED');result=profile.cascadeStoryConsent(tx,awardee,e.auth);});return e.json(200,result);
}, $apis.requireAuth('users'));
