...
onRecordCreateRequest((e) => {
	...
	e.record.set('reviewer', ''); e.record.set('reviewNote', ''); e.record.set('reviewedAt', ''); e.record.set('awardedPoints', 0);
...
onRecordUpdateRequest((e) => {
	...
	for (const field of ['owner','awardeeId','activityType','status','revisionCount','reviewer','reviewNote','reviewedAt','awardedPoints']) {
...
routerAdd('POST', '/api/pfriends/activity-submissions/{id}/review', (e) => {
	...
	e.app.runInTransaction((tx) => {
		...
		if (submission.getString('status') !== 'SUBMITTED') throw new BadRequestError('Pengajuan sudah diputuskan atau belum dikirim ulang.');
		const now = new Date().toISOString();
		const reviews = tx.findCollectionByNameOrId('submission_reviews');
		const review = new Record(reviews);
		review.set('submission', id); review.set('reviewer', e.auth.id); review.set('decision', body.decision); review.set('note', String(body.note || '').trim()); review.set('decidedAt', now);
		tx.save(review);
