...
const sobiPb=new PocketBase(url);await sobiPb.collection('users').authWithPassword(sobi.email,SANDI_DEMO);let sobiBlocked=false;try{await sobiPb.send('/api/pfriends/profile/products',{method:'POST',body:form});}catch(error){sobiBlocked=error.status===403;}ok(sobiBlocked,'Awardee SOBI dapat membuat produk.');
