...
if (process.env.ALLOW_DEMO_SEED !== '1') {
	throw new Error('Seeder demo diblokir. Set ALLOW_DEMO_SEED=1 hanya untuk database lokal atau test sementara.');
}
if (process.env.APP_ENV === 'production' || parsedUrl.hostname === 'friend-api.pertaminafoundation.org') {
	throw new Error('Seeder demo tidak boleh dijalankan terhadap production.');
}
...
const awardeeSeedById = new Map(awardees.map((awardee) => [awardee.id, awardee]));
let created = 0;
...
for (const account of accounts) {
	...
	const data = { email: account.email, emailVisibility: false, password: SANDI_DEMO, passwordConfirm: SANDI_DEMO, legacyAccountId: account.id, awardeeId: account.awardeeId || '', role: account.role, community: awardeeSeedById.get(account.awardeeId)?.community || '', displayName: account.displayName, unit: account.unit || '', status: account.status, onboardingStatus: 'APPROVED', lastLoginAt: account.lastLoginAt || null };
	const user = existing
